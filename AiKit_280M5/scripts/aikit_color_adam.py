...
class Object_detect:
    def __init__(self, camera_x=160, camera_y=15):
        ...
        self.num_block = 0
        self.block_xs = np.array([])
        self.block_ys = np.array([])
        self.block_colors = np.array([])
    ...
    def move(self, x, y, color):
        ...
        if self.num_block < 2:
            # 且沒有重複的立方體
            if np.sum(self.block_colors == color) == 0:
                # 添加立方體
                self.num_block = self.num_block + 1
                self.block_xs = np.append(self.block_xs, x)
                self.block_ys = np.append(self.block_ys, y)
                self.block_colors = np.append(self.block_colors, color)
                print("找到 {} 立方體，共 {} 個".format(self.color_dict[color], self.num_block))
            else:
                # 如果有重複的立方體，則不添加
                return
        # 如果累積的立方體數量等於2個，則開始執行推疊動作
            ...
            # red 0
            print("找到2個立方體")
            if self.block_colors[0] > self.block_colors[1]:
                top_color = self.block_colors[0]
                top_x = self.block_xs[0]
                top_y = self.block_ys[0]
                base_color = self.block_colors[1]
                base_x = self.block_xs[1]
                base_y = self.block_ys[1]
            else:
                top_color = self.block_colors[1]
                top_x = self.block_xs[1]
                top_y = self.block_ys[1]
                base_color = self.block_colors[0]
                base_x = self.block_xs[0]
                base_y = self.block_ys[0]

            print(
                "開始將 {} 放到 {} 的上面".format(
                    self.color_dict[top_color], self.color_dict[base_color]
                )
            )
            ...
            print("移動到 {} 的上面".format(self.color_dict[top_color]))
            # send coordinates to move mycobot
            self.mc.send_coords(
                [top_x, top_y, 170.6, 179.87, -3.78, -62.75], 40, 1
            )  # usb :rx,ry,rz -173.3, -5.48, -57.9
            time.sleep(3)

            self.mc.send_coords([top_x, top_y, 103, 179.87, -3.78, -62.75], 40, 1)
            time.sleep(3)

            print("吸取 {} 立方體".format(self.color_dict[top_color]))
            # open pump
            self.pump_on()
            time.sleep(1.5)
            ...
            print("提起 {} 立方體".format(self.color_dict[top_color]))
            self.mc.send_angles(
                [tmp[0], -0.71, -54.49, -23.02, -0.79, tmp[5]], 25
            )  # [18.8, -7.91, -54.49, -23.02, -0.79, -14.76]
            time.sleep(3)

            print("移動到 {} 的上面".format(self.color_dict[base_color]))
            self.mc.send_coords(
                [base_x, base_y, 170.6, 179.87, -3.78, -62.75], 40, 1
            )  # usb :rx,ry,rz -173.3, -5.48, -57.9
            time.sleep(3)

            print("放下 {} 立方體".format(self.color_dict[top_color]))
            self.pump_off()
            time.sleep(1.5)
    ...
    def color_detect(self, img):
        ...
        for mycolor, item in self.HSV.items():
            ...
            # if the color has been detected, skip it
            if mycolor == "yellow" and np.sum(self.block_colors == 1) > 0:
                continue
            elif mycolor == "red" and np.sum(self.block_colors == 0) > 0:
                continue
            elif mycolor == "cyan" and np.sum(self.block_colors == 3) > 0:
                continue
            elif mycolor == "blue" and np.sum(self.block_colors == 3) > 0:
                continue
            elif mycolor == "green" and np.sum(self.block_colors == 2) > 0:
                continue
...
if __name__ == "__main__":
    ...
    if platform.system() == "Windows":
        cap_num = 1
        cap = cv2.VideoCapture(cap_num, cv2.CAP_ANY)
